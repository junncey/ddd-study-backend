package com.example.ddd.infrastructure.persistence.handler;

import com.example.ddd.domain.model.valueobject.StorageType;
import org.apache.ibatis.type.BaseTypeHandler;
import org.apache.ibatis.type.JdbcType;
import org.apache.ibatis.type.MappedTypes;

import java.sql.CallableStatement;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

/**
 * StorageType 枚举的 MyBatis TypeHandler
 * 负责 StorageType 与 TINYINT 之间的转换
 *
 * @author DDD Demo
 */
@MappedTypes(StorageType.class)
public class StorageTypeHandler extends BaseTypeHandler<StorageType> {

    @Override
    public void setNonNullParameter(PreparedStatement ps, int i, StorageType parameter, JdbcType jdbcType) throws SQLException {
        ps.setInt(i, parameter.getValue());
    }

    @Override
    public StorageType getNullableResult(ResultSet rs, String columnName) throws SQLException {
        Integer value = rs.getInt(columnName);
        return convertToStorageType(value, rs.wasNull());
    }

    @Override
    public StorageType getNullableResult(ResultSet rs, int columnIndex) throws SQLException {
        Integer value = rs.getInt(columnIndex);
        return convertToStorageType(value, rs.wasNull());
    }

    @Override
    public StorageType getNullableResult(CallableStatement cs, int columnIndex) throws SQLException {
        Integer value = cs.getInt(columnIndex);
        return convertToStorageType(value, cs.wasNull());
    }

    private StorageType convertToStorageType(Integer value, boolean wasNull) {
        if (wasNull) {
            return null;
        }
        return StorageType.fromValue(value);
    }
}

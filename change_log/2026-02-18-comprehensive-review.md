...
- **建议方案**:
  1. 后端添加商品图片上传功能
  2. 使用国内可访问的图片占位服务（如 `https://dummyimage.com/`）
  3. 或使用本地默认图片

#### F-2: Ant Design 组件警告 - Drawer
- **位置**: `frontend/src/components/Layout/index.tsx`
- **现象**: `[antd: Drawer] 'width' is deprecated. Please use 'size' instead.`
- **影响**: 控制台警告，未来版本可能不兼容
- **优先级**: P3
- **建议方案**: 将 `width={300}` 改为 `size="default"` 或使用 CSS 控制宽度

#### F-3: Ant Design 组件警告 - Typography
- **位置**: 未定位具体位置
- **现象**: `[antd: Typography.Text] 'ellipsis' do not support 'expandable' or 'rows' props.`
- **影响**: 控制台警告
- **优先级**: P3
- **建议方案**: 检查 Typography.Text 组件的 ellipsis 属性使用方式
...
- **影响**: 用户体验差
- **优先级**: P3
- **建议方案**: 格式化为 `2026-02-18 22:34` 或 `2小时前`
...
| 编号 | 问题 | 预计工时 |
|------|------|---------|
| F-2 | 修复Drawer组件警告 | 0.5h |
| F-3 | 修复Typography组件警告 | 0.5h |
| U-4 | 订单时间格式优化 | 0.5h |
| U-6 | 价格区间筛选 | 2h |
| M-2 | 完善用户中心 | 4h |

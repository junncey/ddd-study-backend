...
#### U-1: 商品列表缺少筛选功能 ✅已修复
- **位置**: `frontend/src/pages/consumer/ProductList/index.tsx`、后端 `ProductApplicationService`
- **现象**: 分类筛选下拉框存在但功能不完善
- **影响**: 用户无法按分类筛选商品
- **优先级**: P2
- **修复内容**:
  1. 后端新增 `findByCategoryIds` 方法支持多分类查询
  2. 后端实现层级查询：选择父分类时自动包含子分类商品
  3. 前端分类下拉框与后端接口对接
- **测试验证**: ✅ 通过 MCP 浏览器测试（选择"电子产品"分类正确显示2件商品）
...
- **优先级**: P2
- **修复内容**:
  1. 后端新增 ReviewController、ReviewApplicationService
  2. 前端新增 ProductReviews 组件、评价 API
  3. 商品详情页集成评价组件
  4. 支持评价统计（好评率、评价数、好评数）
  5. 支持评价列表、商家回复、用户追评
- **测试验证**: ✅ 通过 MCP 浏览器测试（商品详情页显示评价模块）
...
- **修复内容**:
  ...
  2. 实现前端价格区间筛选逻辑
  3. 添加清除筛选按钮
- **测试验证**: ✅ 通过 MCP 浏览器测试（输入最低价 3000 正确过滤出 5999 的商品）
...
- **优先级**: P2
- **修复内容**:
  1. SKU 按钮显示规格名称、规格属性、价格
  2. 售罄 SKU 显示禁用状态和"售罄"标签
  3. 选中 SKU 显示高亮样式
  4. 规格属性从 specs JSON 解析展示
- **测试验证**: 待用户手动测试
